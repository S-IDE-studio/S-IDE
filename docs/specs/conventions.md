...
| カバレッジ | `cargo test` + `pnpm run test:coverage` |
...
| コマンド | 動作 |
|---|---|
| `pnpm run dev` | Web開発サーバー (Vite :5173) |
| `cargo run` | Core Daemon開発起動 |
| `pnpm run dev:desktop` | デスクトップ開発 (Tauri + Vite) |
| `pnpm run build` | 全コンポーネントビルド |
| `pnpm run build:desktop` | デスクトップビルド (web → Core Daemon → Tauri) |
| `pnpm run type-check` | TypeScript型チェック |
| `pnpm run lint` | Biome lint (TS) + clippy (Rust) |
| `pnpm run lint:fix` | Biome自動修正 |
| `pnpm run test` | 全テスト実行 |
| `pnpm run ci` | CI (lint → type-check → test → build) |
