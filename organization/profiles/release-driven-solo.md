...
S-IDE の current release-driven development で使用する Operating Model を定義します。
...
- GitHub Issues / Pull Requests を durable work / review surface とし、未採用の Linear をこの reconciliation だけで canonical 化しない。
...
- `ARCHITECTURE.md` の理念・invariant・domain boundaries と `docs/specs/` を project-specific authority として保持する。
- Context Economy を含む既存 invariant と project-init の progressive disclosure / organizational continuity を両立させる。
- multi-agent parallelism は shared mutable state を無調停で共有しない。
