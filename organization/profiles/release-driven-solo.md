...
- `ARCHITECTURE.md` の principles / invariants と `docs/specs/` の domain boundaries を project-specific authority として維持する。
- context economy / progressive disclosure の既存 design を project-init の Skill / dispatcher architecture と整合させる。
- multi-agent parallel work は domain separation だけで安全とみなさず、mutable runtime ownership / evidence binding も満たす。
