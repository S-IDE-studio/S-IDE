...
export function TerminalTile({ session, wsUrl, onDelete, onError }: TerminalTileProps) {
  ...
  useEffect(() => {
    ...
    term.parser.registerCsiHandler({ prefix: "?", final: "p", intermediates: "$" }, (params) => {
      const mode = (params as any).params[0] || 0;
    ...
    term.parser.registerOscHandler(10, (data) => {
      if (data === "?") {
        ...
        sendResponse(
          `\x1b]10;rgb:${colorTo16BitHex(r)}/${colorTo16BitHex(g)}/${colorTo16BitHex(b)}\x07`
        );
        return true;
      }
      return false;
    ...
    term.parser.registerOscHandler(11, (data) => {
      if (data === "?") {
        ...
        sendResponse(
          `\x1b]11;rgb:${colorTo16BitHex(r)}/${colorTo16BitHex(g)}/${colorTo16BitHex(b)}\x07`
        );
        return true;
      }
      return false;
    ...
    term.parser.registerCsiHandler({ prefix: ">", final: "q" }, (_params) => {
    ...
    term.parser.registerCsiHandler({ prefix: "?", final: "u" }, (_params) => {
    ...
    term.parser.registerCsiHandler({ prefix: "?", final: "m" }, (params) => {
      const param = (params as any).params[0];
      if (param === 4) {
    ...
    term.parser.registerCsiHandler({ final: "t" }, (params) => {
      const operation = (params as any).params[0];
    ...
    term.parser.registerCsiHandler({ prefix: "?", final: "S" }, (_params) => {
    ...
    term.parser.registerDcsHandler({ intermediates: "$", final: "q" }, (data, _params) => {
    ...
    term.parser.registerCsiHandler({ final: "b" }, (params) => {
      const repeatCount = (params as any).params[0] || 1;
  ...
  }, [session.title, wsUrl, onError]);

  return (
    <div className="terminal-tile">
      <div className="terminal-tile-header" style={{ borderLeftColor: session.color || undefined }}>
        <div className="terminal-tile-header-content">
          {session.color && (
            <span className="terminal-color-indicator" style={{ backgroundColor: session.color }} />
          )}
